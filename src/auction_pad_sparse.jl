...
function forward_iteration!(openRows::Queue{G},
                            r2c::Array{G, 1}, c2r::Array{G, 1},
                            rowPrices::Array{T, 1}, colPrices::Array{T, 1}, padPrices::Array{T, 1},
                            lambda::T, epsi::T,
    ...
    if !iszero(r2c[row])
        return forward_iteration!(openRows, r2c, c2r, rowPrices, colPrices, padPrices, lambda, epsi, trewardMatrix, dfltReward, dfltTwo)
    end
    
    ##Find maximum reward - object price
    maxidx, maxval, maxtwo = maxtwo_row(row, colPrices, padPrices, trewardMatrix, dfltReward, dfltTwo)
    padmax = (maxidx == -one(G))
    ...
    if bid >= lambda
        ...
        ##Check if column is already assigned, unassign if necessary this only occurs if the
        #pad assignment is not used as it can only be assigned to the corresponding row
        if padmax
            addassign = true
        else
    ...
    return addassign, openRows, r2c, c2r, rowPrices, colPrices, padPrices
end
...
function reverse_iteration!(openColsAbove::Queue{G},
    ...
    if padcol
        exclude = padPrices[-col] <= lambda || r2c[-col] == col
    else
        exclude = colPrices[col] <= lambda || !iszero(c2r[col])
    end
    if exclude

        ##Recursively call if additional columns above lambda are available
        if length(openColsAbove) > 0
            #return reverse_iteration!(openColsAbove, openColsLeq, r2c, c2r, rowPrices, colPrices, lambda, epsi, rewardMatrix,dfltTwo)
            return reverse_iteration!(openColsAbove, r2c, c2r, rowPrices, colPrices, padPrices, lambda, epsi, rewardMatrix, dfltReward, dfltTwo)
        else
            addassign = false
            #return addassign, openColsAbove, openColsLeq, r2c, c2r, rowPrices, colPrices
            return addassign, openColsAbove, r2c, c2r, rowPrices, colPrices, padPrices
        end
    end
    
    ##Find maximum reward - object price
    maxidx, maxval, maxtwo = maxtwo_column(col, rowPrices, rewardMatrix, dfltReward, dfltTwo)
    ...
    if maxval >= (lambda + epsi)
        ...
        if twoMinusEpsilon > lambda
            if padcol
                padPrices[-col] = twoMinusEpsilon
                rowPrices[maxrow] = dfltReward - twoMinusEpsilon
            else
                colPrices[col] = twoMinusEpsilon
                rowPrices[maxrow] = rewardMatrix.nzval[maxidx] - twoMinusEpsilon
            end
        else
            if padcol
                padPrices[-col] = lambda
                rowPrices[maxrow] = dfltReward - lambda
            else
                colPrices[col] = lambda
                rowPrices[maxrow] = rewardMatrix.nzval[maxidx] - lambda
            end
        end
...
function forward_iteration_counting!(nbelow::G, openRows::Queue{G},
    ...
    ##Row may have been assigned in a reverse iteration, assumes another free row since otherwise all rows are already assigned...
    if !iszero(r2c[row])
        return forward_iteration_counting!(nbelow, openRows, r2c, c2r, rowPrices, colPrices, padPrices, lambda, epsi, trewardMatrix, dfltReward, dfltTwo)
    end
    ...
    return addassign, nbelow, openRows, r2c, c2r, rowPrices, colPrices, padPrices
end
...
function reverse_iteration_counting!(nbelow::G, openColsAbove::Queue{G},
    ...
    if exclude
        ...
        if length(openColsAbove) > 0
            #return reverse_iteration!(openColsAbove, openColsLeq, r2c, c2r, rowPrices, colPrices, lambda, epsi, rewardMatrix,dfltTwo)
            return reverse_iteration_counting!(nbelow, openColsAbove, r2c, c2r, rowPrices, colPrices, padPrices, lambda, epsi, rewardMatrix, dfltReward, dfltTwo)
        else
            addassign = false
            #return addassign, openColsAbove, openColsLeq, r2c, c2r, rowPrices, colPrices
            return addassign, nbelow, openColsAbove, r2c, c2r, rowPrices, colPrices, padPrices
        end
    end
...
function scaling_as(r2c::Array{G, 1}, c2r::Array{G, 1},
                    rowPrices::Array{T, 1}, colPrices::Array{T, 1}, padPrices::Array{T, 1},
                    lambda::T, epsi::T,
                    rewardMatrix::SparseMatrixCSC{T, G}, trewardMatrix::SparseMatrixCSC{T, G},
                    dfltReward::T, dfltTwo::T) where {G <: Integer, T <: AbstractFloat}
    ...
    while nassigned < rewardMatrix.m
        addassign, openRows, r2c, c2r, rowPrices, colPrices, padPrices = forward_iteration!(openRows, r2c, c2r, rowPrices, colPrices, padPrices, lambda, epsi, trewardMatrix, dfltReward, dfltTwo)
        if addassign
            nassigned += one(G)
        end
    end
    ...
    if rewardMatrix.m > 1
        for ii in 2:rewardMatrix.m
            if r2c[ii] < zero(G)
                if padPrices[ii] < lambda
                    lambda = padPrices[ii]
                end
            else
    ...
    for ii in 1:rewardMatrix.m
        if padPrices[ii] > lambda
            enqueue!(openColsAbove, -ii)
        end
    end
    
    ##Execute reverse iterations until termination
    while length(openColsAbove) > 0
        addassign, openColsAbove, r2c, c2r, rowPrices, colPrices, padPrices = reverse_iteration!(openColsAbove, r2c, c2r, rowPrices, colPrices, padPrices, lambda, epsi, rewardMatrix, dfltReward, dfltTwo)
    end
    
    return r2c, c2r, rowPrices, colPrices, padPrices, lambda
end

function auction_assignment_pad_as(r2c::Array{G, 1}, c2r::Array{G, 1},
                               rowPrices::Array{T, 1}, colPrices::Array{T, 1}, padPrices::Array{T, 1},
                               rewardMatrix::SparseMatrixCSC{T, G},
...
function auction_assignment_pad_as(rewardMatrix::SparseMatrixCSC{T, G},
                               epsi0::T = one(T), epsitol::T = T(one(T) / size(rewardMatrix, 1)), epsiscale::T = T(0.2),
                               dfltReward::T = zero(T), dfltTwo::T = -T(Inf)) where {G <: Integer, T <: AbstractFloat}
    ...
    padPrices = zeros(T, size(rewardMatrix, 1))
    return auction_assignment_pad_as(r2c, c2r, rowPrices, colPrices, padPrices, rewardMatrix, epsi0, epsitol, epsiscale, dfltReward, dfltTwo)
end
...
function scaling_asfr1(r2c::Array{G, 1}, c2r::Array{G, 1},
    ...
    for ii in 1:rewardMatrix.m
        if padPrices[ii] > lambda
            enqueue!(openColsAbove, -ii)
        elseif padPrices[ii] < lambda
            nbelow += one(G)
        end
    end
    
    while nassigned < rewardMatrix.m || length(openColsAbove) > 0
        ...
        if nassigned < rewardMatrix.m
            ...
            addassign = false
            while !addassign
                addassign, nbelow, openRows, r2c, c2r, rowPrices, colPrices, padPrices = forward_iteration_counting!(nbelow, openRows, r2c, c2r, rowPrices, colPrices, padPrices, lambda, epsi, trewardMatrix, dfltReward, dfltTwo)
            end
            nassigned += one(G)
        end
        ...
        while !addassign && length(openColsAbove) > 0
            ...
            addassign, nbelow, openColsAbove, r2c, c2r, rowPrices, colPrices, padPrices = reverse_iteration_counting!(nbelow, openColsAbove, r2c, c2r, rowPrices, colPrices, padPrices, lambda, epsi, rewardMatrix, dfltReward, dfltTwo)
        end
        if addassign
            nassigned += one(G)
        end
        ...
        if nbelow > nexcesscols
            combPrices = [colPrices; padPrices]
            colperm = sortperm(combPrices)
            newlambda = combPrices[colperm[nexcesscols]]
            ...
            for jj in 1:rewardMatrix.n
                if colPrices[jj] > newlambda && colPrices[jj] <= lambda
                    enqueue!(openColsAbove, jj)
                end
            end

            for ii in 1:rewardMatrix.m
                if padPrices[ii] > newlambda && padPrices[ii] <= lambda
                    enqueue!(openColsAbove, -ii)
                end
            end
...
function auction_assignment_pad_asfr1(r2c::Array{G, 1}, c2r::Array{G, 1},
                                  rowPrices::Array{T, 1}, colPrices::Array{T, 1}, padPrices::Array{T, 1},
                                  rewardMatrix::SparseMatrixCSC{T, G},
                                  lambda0::T, epsi0::T = one(T), epsitol::T = T(one(T) / size(rewardMatrix, 1)), epsiscale::T = T(0.1),
                                  dfltReward::T = zero(T), dfltTwo::T = -T(Inf)) where {G <: Integer, T <: AbstractFloat}
    trewardMatrix = permutedims(rewardMatrix, [2, 1])
    epsi = epsi0
    lambda = lambda0
    ...
    while abovetol
         ...
         r2c, c2r, rowPrices, colPrices, padPrices, lambda = scaling_asfr1(r2c, c2r, rowPrices, colPrices, padPrices, lambda, epsi, rewardMatrix, trewardMatrix, dfltReward, dfltTwo)
...
function auction_assignment_pad_asfr1(rewardMatrix::SparseMatrixCSC{T, G},
    ...
    padPrices = fill(-dfltReward, size(rewardMatrix, 1))
    if lambda0 < zero(T)
        lambda0 = sort([colPrices; padPrices])[rewardMatrix.n]
    end
    return auction_assignment_pad_asfr1(r2c, c2r, rowPrices, colPrices, padPrices, rewardMatrix, lambda0, epsi0, epsitol, epsiscale, dfltReward, dfltTwo)
end
...
function auction_assignment_pad_asfr2(r2c::Array{G, 1}, c2r::Array{G, 1},
                                  rowPrices::Array{T, 1}, colPrices::Array{T, 1}, padPrices::Array{T, 1},
                                  rewardMatrix::SparseMatrixCSC{T, G},
    ...
    while abovetol
         ...
         r2c, c2r, rowPrices, colPrices, padPrices, lambda = scaling_asfr2(r2c, c2r, rowPrices, colPrices, padPrices, lambda, epsi, rewardMatrix, trewardMatrix, dfltReward, dfltTwo)
...
function auction_assignment_pad_asfr2(rewardMatrix::SparseMatrixCSC{T, G},
                                  epsi0::T = one(T), epsitol::T = T(one(T) / size(rewardMatrix, 1)), epsiscale::T = T(0.1),
                                  dfltReward::T = zero(T), dfltTwo::T = -T(Inf)) where {G <: Integer, T <: AbstractFloat}
    r2c = zeros(G, size(rewardMatrix, 1))
    c2r = zeros(G, size(rewardMatrix, 2))
    ...
    colPrices = zeros(T, size(rewardMatrix, 2))
    padPrices = zeros(T, size(rewardMatrix, 1))
    return auction_assignment_pad_asfr2(r2c, c2r, rowPrices, colPrices, padPrices, rewardMatrix, epsi0, epsitol, epsiscale, dfltReward, dfltTwo)
end
