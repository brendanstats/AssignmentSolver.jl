...
function forward_iteration!(openRows::Queue{G},
    ...
    if !iszero(r2c[row])
        return forward_iteration!(openRows, r2c, c2r, rowPrices, colPrices, lambda, epsi, trewardMatrix, dfltTwo)
    end
...
function reverse_iteration!(openColsAbove::Queue{G},
    ...
    if colPrices[col] <= lambda || !iszero(c2r[col])
        ...
        if length(openColsAbove) > 0
            ...
            return reverse_iteration!(openColsAbove, r2c, c2r, rowPrices, colPrices, lambda, epsi, rewardMatrix, dfltTwo)
        else
            addassign = false
...
function forward_iteration_counting!(nbelow::G, openRows::Queue{G},
    ...
    ##Row may have been assigned in a reverse iteration, assumes another free row since otherwise all rows are already assigned...
    if !iszero(r2c[row])
        return forward_iteration_counting!(nbelow, openRows, r2c, c2r, rowPrices, colPrices, lambda, epsi, trewardMatrix, dfltTwo)
    end
...
function reverse_iteration_counting!(nbelow::G, openColsAbove::Queue{G},
    ...
    if colPrices[col] <= lambda || !iszero(c2r[col])
        ...
        if length(openColsAbove) > 0
            #return reverse_iteration!(openColsAbove, openColsLeq, r2c, c2r, rowPrices, colPrices, lambda, epsi, rewardMatrix,dfltTwo)
            return reverse_iteration_counting!(nbelow, openColsAbove, r2c, c2r, rowPrices, colPrices, lambda, epsi, rewardMatrix, dfltTwo)
        else
            addassign = false
            #return addassign, openColsAbove, openColsLeq, r2c, c2r, rowPrices, colPrices
...
function scaling_as(r2c::Array{G, 1}, c2r::Array{G, 1},
    ...
    while nassigned < rewardMatrix.m
        addassign, openRows, r2c, c2r, rowPrices, colPrices = forward_iteration!(openRows, r2c, c2r, rowPrices, colPrices, lambda, epsi, trewardMatrix, dfltTwo)
        if addassign
            nassigned += one(G)
        end
    end
    ...
    while length(openColsAbove) > 0
        addassign, openColsAbove, r2c, c2r, rowPrices, colPrices = reverse_iteration!(openColsAbove, r2c, c2r, rowPrices, colPrices, lambda, epsi, rewardMatrix, dfltTwo)
    end
...
function auction_assignment_as(rewardMatrix::SparseMatrixCSC{T, G},
                               epsi0::T = one(T), epsitol::T = T(one(T) / size(rewardMatrix, 1)), epsiscale::T = T(0.2),
                               dfltTwo::T = -T(Inf)) where {G <: Integer, T <: AbstractFloat}
    ...
    return auction_assignment_as(r2c, c2r, rowPrices, colPrices, rewardMatrix, epsi0, epsitol, epsiscale, dfltTwo)
end
...
function scaling_asfr1(r2c::Array{G, 1}, c2r::Array{G, 1},
                       ...
                       rewardMatrix::SparseMatrixCSC{T, G}, trewardMatrix::SparseMatrixCSC{T, G},
                       dfltTwo::T) where {G <: Integer, T <: AbstractFloat}
    nexcesscols = rewardMatrix.n - rewardMatrix.m
    ...
    while nassigned < rewardMatrix.m || length(openColsAbove) > 0
        ...
        if nassigned < rewardMatrix.m
            ...
            addassign = false
            while !addassign
                addassign, nbelow, openRows, r2c, c2r, rowPrices, colPrices = forward_iteration_counting!(nbelow, openRows, r2c, c2r, rowPrices, colPrices, lambda, epsi, trewardMatrix, dfltTwo)
            end
            nassigned += one(G)
        end
        ...
        while !addassign && length(openColsAbove) > 0
            ...
            addassign, nbelow, openColsAbove, r2c, c2r, rowPrices, colPrices = reverse_iteration_counting!(nbelow, openColsAbove, r2c, c2r, rowPrices, colPrices, lambda, epsi, rewardMatrix, dfltTwo)
        end
        if addassign
            nassigned += one(G)
        end
        ...
        if nbelow > nexcesscols
            ...
            jj = nexcesscols + 1
            while (jj <= rewardMatrix.n) && (colPrices[colperm[jj]] <= lambda)
...
function auction_assignment_asfr1(r2c::Array{G, 1}, c2r::Array{G, 1},
    ...
    while abovetol
         ...
         r2c, c2r, rowPrices, colPrices, lambda = scaling_asfr1(r2c, c2r, rowPrices, colPrices, lambda, epsi, rewardMatrix, trewardMatrix, dfltTwo)
...
function auction_assignment_asfr1(rewardMatrix::SparseMatrixCSC{T, G},
    ...
    if (lambda0 < zero(T)) && (rewardMatrix.n - rewardMatrix.m) > zero(G)
        lambda0 = sort(colPrices)[rewardMatrix.n - rewardMatrix.m]
    end
    return auction_assignment_asfr1(r2c, c2r, rowPrices, colPrices, rewardMatrix, lambda0, epsi0, epsitol, epsiscale, dfltTwo)
end
...
function auction_assignment_asfr2(r2c::Array{G, 1}, c2r::Array{G, 1},
    ...
    while abovetol
         ...
         r2c, c2r, rowPrices, colPrices, lambda = scaling_asfr2(r2c, c2r, rowPrices, colPrices, lambda, epsi, rewardMatrix, trewardMatrix, dfltTwo)
...
function auction_assignment_asfr2(rewardMatrix::SparseMatrixCSC{T, G},
                                  epsi0::T = one(T), epsitol::T = T(one(T) / size(rewardMatrix, 1)), epsiscale::T = T(0.1),
                                  dfltTwo::T = -T(Inf)) where {G <: Integer, T <: AbstractFloat}
    r2c = zeros(G, size(rewardMatrix, 1))
    c2r = zeros(G, size(rewardMatrix, 2))
    rowPrices = vec(maximum(rewardMatrix, dims = 2))
    colPrices = zeros(T, size(rewardMatrix, 2))
    return auction_assignment_asfr2(r2c, c2r, rowPrices, colPrices, rewardMatrix, epsi0, epsitol, epsiscale, dfltTwo)
end
